...
from extractor import BasicEncoder, SmallEncoder, Feature_extractor
from corr import CorrBlock, AlternateCorrBlock
from utils.utils import bilinear_sampler, coords_grid, upflow8
from utils_other import gen_checkerboard
...
class RAFT(nn.Module):
    def __init__(self,smooth_loss='2nd',smooth_mask='none',semantic_loss=False,seq_gamma=0.1):
        ...
        if args.small:
            ...
            # self.fnet = Feature_extractor(requires_grad=True)
    ...
    def forward(self, image1, image2, refine_time=12, flow_init=None, upsample=True, test=False,image1_mask=None,image2_mask=None,corr_map=None):
        ...
        # pos_image1 = image1 + self.pos_embed_img.to(image1.device)
        # pos_image2 = image2 + self.pos_embed_img.to(image2.device)
        # save_image(pos_image1.float().cpu(),'image1_pos.jpg',scale_each=True,nrow=image1.shape[0],padding=0,pad_value=255)     
        # save_image(pos_image2.float().cpu(),'image2_pos.jpg',scale_each=True,nrow=image2.shape[0],padding=0,pad_value=255)   

        # save_image(image1.float().cpu(),'image1_vis.jpg',scale_each=True,nrow=image1.shape[0],padding=0,pad_value=255)     
        # save_image(image2.float().cpu(),'image2_vis.jpg',scale_each=True,nrow=image2.shape[0],padding=0,pad_value=255)        
