...
# def sequence_L1_loss_with_TV(warped_I_c_list,I_g,gamma=0.8,warp_fields=None):
...
#             # ell_warp_TV += i_weight * second_order_smooth_loss(warp_fields[i])
#     if warp_fields is not None:
#         return ell_warp, ell_warp_TV
#     else:
#         return ell_warp
...
def sequence_L1_p_loss(warped_I_c_list,I_g,gamma=0.8):
    ...
    for i in range(n_predictions):
        ...
        # i_weight = [1,1,1,1][i]
        # i_weight = torch.linspace(0.01,1,8)[n_predictions+i]
...
def TV_with_mask(x,mask,alpha=0.9999,mask_type='binary'):
    ...
    # alpha = alpha/(1-alpha) if alpha!=1 else 1e9
    # n_1 = mask.flatten(2).sum(dim=2).view(mask.shape[0],1,1,1)  # area_size
    # n = x.shape[2] * x.shape[3]
    # n_2 = n - n_1 # bg_size
    # alpha1 = (alpha*n) / (alpha*n_1 + n_2)
    # alpha2 = n / (alpha*n_1 + n_2)
    # t = torch.pow(torch.abs(x[:,:,1:,: ] - x[:,:,0:-1,:  ]), 2)
    # ell = (alpha1 * t * mask[:,:,1:,:] + alpha2 * t * (~mask[:,:,1:,:])).mean()
    # t = torch.pow(torch.abs(x[:,:,: ,1:] - x[:,:,:  ,0:-1]), 2)
    # ell += (alpha1 * t * mask[:,:,: ,1:] + alpha2 * t * (~mask[:,:,: ,1:])).mean()
    # t = torch.pow(torch.abs(x[:,:,1:,1:] - x[:,:, :-1, :-1]), 2)
    # ell += (alpha1 * t * mask[:,:,1:,1:] + alpha2 * t * (~mask[:,:,1:,1:])).mean()
    # t = torch.pow(torch.abs(x[:,:,1:,:-1] - x[:,:,:-1,1:]), 2)
    # ell += (alpha1 * t * mask[:,:,1:,:-1] + alpha2 * t * (~mask[:,:,1:,:-1])).mean()
    # ell /= 4.
    return ell
...
def sequence_TV_loss(warp_fields,gamma=0.1):
    ...
    for i in range(n_predictions):
        ...
        # i_weight = gamma**(n_predictions - i - 1) * ((n_predictions-i)/2.5)
        # i_weight = gamma**i
        # i_weight = 1 / n_predictions
        # i_weight = [0.01,0.01,0.01,0.01][i]
        # i_weight = torch.linspace(0.01,1,8)[n_predictions-i-1]
